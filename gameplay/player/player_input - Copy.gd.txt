...
var throttle_up_pressed: bool = false
var throttle_down_pressed: bool = false

var mouse_input_pos: Vector2
...
func get_combined_input() -> void:
	...
	throttle_up_pressed = Input.is_action_pressed("throttle_up")
	throttle_down_pressed = Input.is_action_pressed("throttle_down")
...
func get_mouse_input() -> void:
	# use our virtual mouse position instead of the real cursor
	var min_bounds : Vector2 = input_center - input_half_extents
	var max_bounds : Vector2 = input_center + input_half_extents

	# clamp virtual mouse to input box
	mouse_input_pos = mouse_input_pos.clamp(min_bounds, max_bounds)

	var offset : Vector2 = mouse_input_pos - input_center
	var input_vector : Vector2 = Vector2(
		offset.x / input_half_extents.x,
		offset.y / input_half_extents.y
	)

	if input_vector.length() < deadzone_prcnt:
		input_vector = Vector2.ZERO # inside deadzone, remove input
...
func _recalc_inputbox() -> void:
	...
	mouse_input_pos = input_center  # keep our virtual mouse centered on resize

func _input(event: InputEvent) -> void:
	# check if we're using mouse and keyboard
	if event is InputEventMouseMotion:
		gamepad = false
		# move our virtual mouse using relative motion
		mouse_input_pos += event.relative
